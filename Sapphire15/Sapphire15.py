# ----------------------------------------------------------------------
# Sapphire Engine v1.5: Stable Basic Engine (Fixed for Pygame & UTF-8)
...
def execute_line(line, locals=None):
    ...
    if line.startswith('draw_rect'):
        if not screen: error("Must call set_screen before drawing!")
        c, x, y, w, h = re.match(r'draw_rect\s*\(\s*(\w+),\s*(.*),\s*(.*),\s*(.*),\s*(.*)\s*\)', line).groups()
        pygame.draw.rect(screen, COLOR_MAP.get(c.upper(), (255,255,255)), pygame.Rect(int(eval_expression(x, locals=locals)), int(eval_expression(y, locals=locals)), int(eval_expression(w, locals=locals)), int(eval_expression(h, locals=locals)))); return
