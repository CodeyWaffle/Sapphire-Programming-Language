# ----------------------------------------------------------------------
# Sapphire Studio v1.1: Stable Basic Engine (more Pygame shapes and abort() command)
...
class SapphireIDE:
    def __init__(self, root):
        ...
        self.root.title(" Sapphire Studio v1.1")
    ...
    def detect_version(self):
        files = [f for f in os.listdir('.') if f.startswith('Sapphire') and f.endswith('.py') and f != 'SapphireStudio11.py']
        if not files: return "16" # Default to latest version
        latest = sorted(files, reverse=True)[0]
        return latest.replace('Sapphire', '').replace('.py', '')

    def load_welcome_code(self):
        welcome = """// 💎 Welcome to Sapphire v1.6
setup {
    //this is a comment.
